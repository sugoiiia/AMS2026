...
// ---------------------------------------------------------------
#define BASE_SPEED         150   // Default drive speed (0-255). Start here, increase if carpet is too slow.
#define TURN_SPEED         160   // Speed during turns. Slightly higher than base to overcome carpet friction.
#define REVERSE_SPEED      130   // Speed when reversing away from front collision.

#define WALL_FOLLOW_DIST   10    // Target distance (cm) to maintain from left wall.
#define WALL_TOO_CLOSE     6     // Left wall too close — steer right.
#define WALL_TOO_FAR       15    // Left wall too far / lost — steer left.
#define FRONT_CLEAR_DIST   20    // (unused — IR is digital, kept for reference)

#define REVERSE_TIME       300   // ms to reverse after front collision before turning.
#define TURN_TIME          500   // ms to turn right after front collision. Tune this for 90 degrees on carpet.
...
void loop() {
  ...
  if (frontWall) {
    Serial.println("ACTION: Front wall — reversing and turning right");
    stopMotors();
    delay(100);
...
// Full turn left in place (right wheels forward, left wheels backward)
...
// Gentle steer left — right side faster than left
void steerLeft() {
  digitalWrite(back_left_1,   HIGH); digitalWrite(back_left_2,   LOW);
  digitalWrite(back_right_1,  HIGH); digitalWrite(back_right_2,  LOW);
  digitalWrite(front_left_1,  LOW);  digitalWrite(front_left_2,  HIGH);
